...
public class ReservaBO {
     ...
     * @return ID de la reserva creada, 0 si falló, -1 si se agregó a lista de espera
     */
    ...
    private boolean sonHorariosConflictivos(Date hora1, Date hora2) {
        if (hora1 == null || hora2 == null) return false;
    ...
    private void validarPermisosConfirmacion(UsuariosDTO usuario) {
        Integer rolId = usuario.getRol().getIdRol();
        
        // Solo Administrador (1), Mozo (2) y Gerente (5) pueden confirmar
        if (rolId != 1 && rolId != 2 && rolId != 5) {
            throw new RuntimeException("No tiene permisos para confirmar reservas");
        }
    }
    ...
    private boolean sonFechasIguales(Date fecha1, Date fecha2) {
        if (fecha1 == null && fecha2 == null) return true;
